...
if (isset($_GET['id_boleta'])) {
    ...
    if ($tarifa_column_exists) {
        $sql = "SELECT b.ID_BOLETA, b.FECHA_ENTRADA, b.FECHA_SALIDA, b.MONTO, c.PLACA, t.NOMBRE AS TIPO, t.TARIFA
                FROM tbboletas b
                INNER JOIN tbclientes c ON b.ID_CLIENTE = c.ID_CLIENTE
                INNER JOIN tbtipos t ON c.TIPO = t.ID_TIPO
                WHERE b.ID_BOLETA = ?";
    } else {
        $sql = "SELECT b.ID_BOLETA, b.FECHA_ENTRADA, b.FECHA_SALIDA, b.MONTO, c.PLACA, t.NOMBRE AS TIPO
                FROM tbboletas b
                INNER JOIN tbclientes c ON b.ID_CLIENTE = c.ID_CLIENTE
                INNER JOIN tbtipos t ON c.TIPO = t.ID_TIPO
                WHERE b.ID_BOLETA = ?";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // Verificar si la boleta ya tiene fecha de salida
        if (empty($row['FECHA_SALIDA'])) {
            echo "Esta boleta no ha sido pagada todavía. Por favor, procese el pago primero.";
            echo "<br><a href='caja.php'>Volver a caja</a>";
            exit();
        }
        ...
        // Calcular subtotal e IGV a partir del total
        $subtotal = $total / 1.18;
        $igv = $total - $subtotal;
        ...
        if ($result_serie && mysqli_num_rows($result_serie) > 0) {
            ...
            $numero = $row_serie['NUMERO']; // Usar el número actual, ya se incrementó al pagar
        }
